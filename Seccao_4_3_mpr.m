function [ypdf_idle, success_wifi_time, success_laa_time, matrix, matrix_wifi, matrix_laa, matrix_wifi_norm, matrix_laa_norm, matrix_ocr_norm, meanSINR_totdB] ...
    = mpr(n_wifi, n_laa, simulation_time, CP, xpdf_idle)
...
filename = sprintf('Results_Simul_100011vPaper_RG0_RE10_b50_PxdB20_noise25_Shad69_PL200.mat');
load(filename);
% Psucc_wifi
% Psucc_laa
...
while (i<simulation_time)
    ...
    if (n_transmissions > 0)
        ...
        if(n_transmissions > 1)
            ...
            RV_Pwifi = rand(1, n_wifi_nodes);
            RV_Plaa = rand(1, n_laa_nodes);
            Plaa = Psucc_laa(n_wifi_nodes + 1, n_laa_nodes + 1);
            Pwifi = Psucc_wifi(n_wifi_nodes + 1, n_laa_nodes + 1);     
            %Ptotal > RV_Psucc
            success_node = [Pwifi>RV_Pwifi , Plaa>RV_Plaa];

            
            Succ_node = transm_nodes(success_node==1);
            Coll_nodes = transm_nodes(success_node==0);
...
probabilibade=matrix_ocr/sum(sum(matrix_ocr,2), 1)
matrix_ocr(1,2)=0;
matrix_ocr(2,1)=0;
